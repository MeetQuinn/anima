...
4. **The agent worker claims one item.** Each agent drains its own primary work serially. Across the team, at most five provider runs are active at once; additional items remain queued until a slot opens. Follow-up messages can join a compatible active provider turn without consuming another slot.

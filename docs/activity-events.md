...
| `anima.reminder.preflight` (completed)      | Hosted preflight result applied (succeeded/declined/errored). Audit-only; no command or streams.   | Conversation     | `Preflight succeeded                                              | declined    | errored` + title · duration/exit. | —   |

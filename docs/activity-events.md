...
| Planning / delegation: `Agent`, `Skill`, `TaskCreate`, `TaskUpdate`, `TodoWrite`, `SetTodoList` (started)                                                            | Provider starts a helper tool.                      | Conversation | Mapped verbs: `Delegated to subagent` / `Ran skill` / `Created task` / task-status verb / `Updated todos`. Claude task rows include the available subject/active form, task ID, and status. | Full target if truncated.      |
| `runtime.event:codex.plan.updated`                                                                                                                                   | Codex publishes a structured plan update.           | Conversation | `Updated plan` + completed count + current step.                                                                                                                                            | Full step/status list.         |
...
| `anima.reminder.preflight` (completed)      | Hosted preflight result applied (succeeded/declined/errored). Best-effort audit after commit; no command or streams. | Conversation     | `Preflight succeeded` / `declined` / `errored` + title · duration/exit. | —           |
...
| `provider.transient.retry`                                                                                                                                                               | Transient provider API error (5xx/overload, network or stream stall, safeguard false positive); Anima re-sends the same item on the same session after backoff (5s / 30s / 2min, 3 attempts). | Audit                 | `Provider retry` + attempt/max + error.                      | Full error.   |
| `provider.rate_limit.defer`                                                                                                                                                              | Provider rate limit or session quota; Anima parks the item until the reported reset (30s–6h, default 5min) instead of failing it. Bounded to 6 deferrals per item.                            | Audit                 | `Rate limit` + resume time.                                  | Full error.   |
| `runtime.failure_notice`                                                                                                                                                                 | Item failed for good (retries exhausted or terminal error); Anima replied in the originating DM/mention so the requester knows to resend.                                                     | Audit                 | `Failure notice` + reason.                                   | —             |
...
| Streaming frames: `*.stream.*`, `*outputDelta`, `*.thinking.delta`, `*.content.part`, `*.tool.call.part`, `*.tool_result`, hooks, plan display / non-Codex plan updates, diff, turn/step | Raw provider protocol and per-token telemetry.                                                                                                                                                | Hidden                | —                                                            | —             |

...
export function isFirstClassAnimaCliCommand(command: string | undefined): boolean {
  ...
  return new RegExp(
    String.raw`^${withOptionalEnv}anima\s+(?:ask|env\s+(?:set|list|run|source)|message\s+(?:read|send|update|react)|file\s+send|reminder\s+(?:schedule|cancel|snooze|list)|subscription\s+(?:list|mute|unmute))\b`,
  ).test(trimmed);
}

...
test('normalizes Feishu mention keys to readable labels', () => {
  ...
  assert.equal(item?.text, '<mention user_id="ou_anima_bot">Anima</mention> please check');
});
  ...
  try {
    await withAnimaHome(stateDir, async () => {
      ...
      await runMessageSend(
        { agent: 'scout', channel: 'oc_target_chat', text: 'hello <mention user_id="ou_alice">Alice</mention>' },
        {
          createFeishuMessageClient() {
            return {
              async addReaction() {

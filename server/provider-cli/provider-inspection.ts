...
async function inspectClaude(
  ...
  const native =
    /[/\\]\.local[/\\]share[/\\]claude[/\\]versions[/\\][^/\\]+$/.test(
      executable.realPath,
    );
  if (!native) {
    return {
      ...
      realPath: executable.realPath,
      sourceDetail:
        'The active Claude Code binary is not a recognized native install',
      updateMode: 'manual',
    };
  }
  const doctor = await runCommand(executable.path, ['doctor'], {
    ...
    .catch(() => '');
  const autoUpdates = doctor.match(
    /^Auto-updates:\s+(enabled|disabled)$/m,
  )?.[1];
...
async function inspectCodex(
  ...
  if (!executable.realPath.endsWith(suffix)) {
    return {
      ...
      realPath: executable.realPath,
      sourceDetail:
        'The active Codex binary is not a recognized global npm install',
      updateMode: 'manual',
    };
  }
  const prefix = executable.realPath
...
async function resolveExecutable(
  ...
  for (const entry of (env.PATH ?? '').split(delimiter)) {
    ...
    if (seenPaths.has(path) || !(await isAccessible(path, constants.X_OK)))
      continue;
    seenPaths.add(path);
  ...
  return {
    ...first,
    shadowed:
      new Set(candidates.map((candidate) => candidate.realPath)).size > 1,
  };
}
